...
TABLE: [
  // Density reprice pass: wave 1 opens PLURAL (CoD's round-1 lesson is
  // "there is a horde, it is just slow"), and every gun price doubled with
  // the income so the pistol-era window keeps its shape.
  { count: 4, speedMult: 1.0,  entry: { perimeter: 1.0, window: 0 },
    types: { proto_zombie: 1.0 } },
  { count: 5, speedMult: 1.0,  entry: { perimeter: 1.0, window: 0 },
    types: { proto_zombie: 1.0 } },
  { count: 6, speedMult: 1.05, entry: { perimeter: 0.7, window: 0.3 },
    types: { proto_zombie: 1.0 } },
  { count: 7, speedMult: 1.1,  entry: { perimeter: 0.6, window: 0.4 },
    types: { proto_zombie: 0.75, crawler: 0.25 } },
   { count: 8, speedMult: 1.15, entry: { perimeter: 0.5, window: 0.5 },
    types: { proto_zombie: 0.6, crawler: 0.2, sprinter: 0.2 } },
  { count: 9, speedMult: 1.2,  entry: { perimeter: 0.5, window: 0.5 },
  ...
  { count: 10, speedMult: 1.25, entry: { perimeter: 0.5, window: 0.5 },
...
// Wave 8 (pass 15): the spitter debuts, and EXTEND carries this mix
  // forever. 0.125 is not a taste call — at count 11 (density pass) it is
  // 1.375 zombies, flooring to 1: the debut still wins a FLOOR slot
  // outright and never depends on the largest-remainder tie-break.
  //
  // Any share below 1/count (now ~0.091) does depend on that tie-break,
  // and it is decided by float noise, not design — measured at the OLD
  // count 8 during the §22 bite-test: at share 0.05 the remainder
  // 0.40000000000000002 BEAT proto's 0.39999999999999991, so a debut
  // survived by luck. Don't tune this below 1/count without re-checking
  // §22's rounding pin.
  { count: 11, speedMult: 1.3,  entry: { perimeter: 0.5, window: 0.5 },
